...
def average_frame_count(real_fake,language):
  input_path = f'/Users/jiyeong/Desktop/컴공 캡스톤/PolyGlotFake/Dataset/{real_fake}/{language}/*.mp4'  #Input file path, 입력 파일 경로 - 파일 경로 수정!!
...
if __name__ == '__main__':
    output_path = "/Users/jiyeong/Desktop/컴공 캡스톤/PolyGlotFake/Dataset/real_pr/real_pr_8910"  # Output file path - 목적지 파일 경로 , 파일 경로 수정!!

    # 각 작업을 별도 프로세스로 실행
    p1 = Process(target=run_job, args=('real', 'to1', output_path))        # Input file path, 입력 파일 경로 - 파일 경로 수정!!
    p2 = Process(target=run_job, args=('real', 'to2', output_path))        # Input file path, 입력 파일 경로 - 파일 경로 수정!!
    p3 = Process(target=run_job, args=('real', 'to3', output_path))        # Input file path, 입력 파일 경로 - 파일 경로 수정!!
